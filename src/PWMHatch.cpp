...
void Hatch::init(){
    ...
    _em241pwmOutput = new ESP32_FAST_PWM(hatchspeedpin, em241pwmfreq, 0.0f, hatchspeedpwmchannel, hatchpwmresolution);
    ...
    if (_em241pwmOutput){
        boolean setSuccess = _em241pwmOutput->setPWM(hatchspeedpin, em241pwmfreq, 0.0f);
        #ifdef _VERBOSE_
            Serial.println("Setup: _em241pwmOutput exists, setPWM fine: " + String(setSuccess));
        #endif
        setSuccess = _em241pwmOutput->setPWM_DCPercentage_manual(hatchspeedpin,hatchspeedoutput);
        #ifdef _VERBOSE_
            Serial.println("Setup: _em241pwmOutput speed set: " + String(setSuccess));
        #endif
    }
